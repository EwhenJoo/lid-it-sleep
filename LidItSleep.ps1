...
<#
.SYNOPSIS
Inspect Windows lid sleep, or explicitly apply/restore lid policy.
.EXAMPLE
.\LidItSleep.ps1
.EXAMPLE
...
function Initialize-PowerApi {
    if ('LidItSleep.Native' -as [type]) { return }
...
function Get-ActiveSchemeId {
    Initialize-PowerApi
    $pointer = [IntPtr]::Zero
    $code = [LidItSleep.Native]::PowerGetActiveScheme([IntPtr]::Zero, [ref]$pointer)
    ...
    finally { [void][LidItSleep.Native]::LocalFree($pointer) }
}
...
function Get-LidPolicy {
    ...
    [uint32]$ac = 0
    [uint32]$dc = 0
    $code = [LidItSleep.Native]::PowerReadACValueIndex([IntPtr]::Zero, [ref]$scheme, [ref]$group, [ref]$setting, [ref]$ac)
    if ($code -ne 0) { throw "Reading AC lid policy failed: $code" }
    $code = [LidItSleep.Native]::PowerReadDCValueIndex([IntPtr]::Zero, [ref]$scheme, [ref]$group, [ref]$setting, [ref]$dc)
...
switch ($Mode) {
    ...
    'Apply' {
        ...
        if ($BatteryAction -eq 'Hibernate') {
            Initialize-PowerApi
            if (-not [LidItSleep.Native]::IsPwrHibernateAllowed()) { throw 'Hibernation is not available. Inspect powercfg /a and configure it explicitly before retrying.' }
        }
