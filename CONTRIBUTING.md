...
Run `./Test-LidItSleep.ps1`, `./Test-Guard.ps1` and `./Build-Guard.ps1` before submitting code. Tests must not change the host's power settings or put it to sleep. Use `build/LidItSleep.Guard.exe --observe --seconds 60 --state-dir test-output` to verify native sensor notifications without suspending. Keep hardware observations separate from mocked test results. Never treat API acceptance as proof of low-power sleep. Do not commit generated executables or personal guard logs.
