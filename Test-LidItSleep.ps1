...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) ('lid-it-sleep-tests-' + [guid]::NewGuid().ToString('N'))
try {
    ...
    if ($resolvedTestRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) -and
        [IO.Path]::GetFileName($resolvedTestRoot) -match '^lid-it-sleep-tests-[a-f0-9]{32}$' -and
